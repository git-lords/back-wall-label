...
export default function Hero({ heros, currentHero, itemsPerPage }) {
    ...
    useEffect(() => {
        ...
        return () => { setShow(true), clearTimeout(id), resetIsShowing() }
    }, [currentPage])
    ...
    return (
        ...
        <div className='h-full w-full text-white flex'>
            <div key={heros[currentPage].heroId} className='h-full relative'>
                <Transition
                ...
                <div className='absolute h-full w-screen flex flex-col justify-end'>
                    {/* Page select arrows */}
                    ...
                    <div className='h-[10%] flex justify-center gap-6 items-center'>
                        {Array.from({ length: totalPages + 1 }).map((_, index) => (
                            <div key={index}>
                                <button className={`heroButton rounded-full shadow shadow-black h-4 w-4 ${index === currentPage ? 'bg-darkMint' : 'bg-zinc-200'}`} onClick={() => paginate(index)}>
                                </button>
                            </div>
                        ))}
                    </div>
