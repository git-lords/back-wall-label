...
useEffect(()=>{
  ...
  return (
    <div className='w-full fixed flex flex-wrap justify-end'>
      ...
      {/* Drop Down Menu */}
      <div ref={dropDownRef} className={`dropDownMenu ${showDropDown? 'active' : 'inactive'} transition-all flex flex-col items-center bg-zinc-200 dark:bg-zinc-900 dark:text-white w-1/3 md:w-1/5`}>
...
export const DropDownItem = (props) => {
  return (
    <>
      ...
      '>
        <div className='w-1/4 p-2 self-center hidden xs:block'>{props.img}
        </div>
        <NavLink to={`/${props.text}`} className='focus:outline-none w-3/4 self-center'> {props.text} </NavLink>
      </div>
    </>
  )
}